...
export default function Pricing() {
  ...
  if (loading) {
    return (
      <section id="pricing" className="public-section py-10 sm:py-14">
  ...
  return (
    <SectionWrapper id="pricing" className="section-shell py-10 sm:py-14 lg:py-20">
      <div className="mx-auto max-w-7xl px-5 sm:px-6 lg:px-8">
        <motion.div className="mb-6 text-center sm:mb-8" initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, amount: 0.2 }}>
          ...
          <p className="public-section-copy mx-auto mt-4">Transparent pricing designed for every style preference and service depth.</p>
        </motion.div>
        ...
        <div className="mb-5 flex justify-center sm:mb-7">
          <div className="inline-flex rounded-full border border-gold-primary/20 bg-black/40 p-1">
            <button
              type="button"
              className={`min-h-[44px] rounded-full px-6 py-2.5 text-sm font-semibold transition-all ${
        ...
        <AnimatedGrid key={gender} className="grid grid-cols-1 gap-[var(--card-gap)] md:grid-cols-3 auto-rows-fr">
          {pricing[gender].map((plan, index) => (
            <PricingCard key={plan.id} {...plan} index={index} gender={gender} />
          ))}
        </AnimatedGrid>

        <p className="mt-4 text-center text-sm text-gray-500">All packages include consultation and premium product usage.</p>
      </div>
    </SectionWrapper>
  )
}
